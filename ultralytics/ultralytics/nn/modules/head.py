...
class AdaptiveRoiStripBlock(nn.Module):

    def __init__(self, dim, layer_idx):
        super().__init__()
        self.layer_idx = layer_idx
        k = 7 if layer_idx == 0 else (5 if layer_idx == 1 else 3)
        self.conv0 = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        self.strip_conv1 = nn.Conv2d(dim, dim, (1, k), padding=(0, k//2), groups=dim)
        self.strip_conv2 = nn.Conv2d(dim, dim, (k, 1), padding=(k//2, 0), groups=dim)
    ...
    def forward(self, x):
        ...
        enhanced = u + self.strength * (attn - u)
        return enhanced
...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: tuple = (), *args, **kwargs):
        ...
        self.reg_strip = nn.ModuleList([
            AdaptiveRoiStripBlock(dim=x, layer_idx=i) for i, x in enumerate(ch)
        ])
    ...
    def forward(self, x: list[torch.Tensor]) -> list[torch.Tensor] | tuple:
        ...
        for i in range(self.nl):
            reg_feat = self.reg_strip[i](x[i])
            x[i] = torch.cat((self.cv2[i](reg_feat), self.cv3[i](x[i])), 1)
    ...
    def forward_end2end(self, x: list[torch.Tensor]) -> dict | tuple:
        ...
        one2one = [
            torch.cat((self.one2one_cv2[i](self.reg_strip[i](x_detach[i])), self.one2one_cv3[i](x_detach[i])), 1)
            for i in range(self.nl)
        ]
        for i in range(self.nl):
            reg_feat = self.reg_strip[i](x[i])
